          ...
          {skillData.map((item, index) => {
            ...
            return (
              <div className="flex flex-col gap-1" key={index}>
                <p className="px-2 capitalize text-base md:text-lg text-textLightBlack font-medium">
                  {item.name}
                </p>
                <div className="h-3 md:h-5 text-xs md:text-base w-full border-2 border-textLightBlack rounded-3xl">
                  <span
                    style={progressBarWidth}
                    className={`h-full flex text-xs md:text-sm text-white justify-center items-center bg-textLightBlack rounded-l-3xl`}
                  >
                    {`${item.skillPercent}%`}
                  </span>
                </div>
